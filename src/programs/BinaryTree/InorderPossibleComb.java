...
public class InorderPossibleComb {
    ...
    Vector<Node> getTrees(int arr[], int start, int end) {
        ...
        for (int i = start; i <= end; i++) {
            ...
            System.out.println(ltrees + "value " +ltrees.size() + " size left " + i);
            /* Constructing right subtree */
            Vector<Node> rtrees = getTrees(arr, i + 1, end);

            System.out.println(rtrees + "value " +rtrees.size() + " size right " + i);
    ...
    public static void main(String args[]) {
        int in[] = {4, 5, 7};
        int n = in.length;
        InorderPossibleComb tree = new InorderPossibleComb();
